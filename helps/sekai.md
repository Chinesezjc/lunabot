...
- `/pjsk订阅 live music` 订阅日服虚拟Live@提醒功能和新曲上线@提醒功能
...
### 自定义个人信息
`/自定义个人信息` `/cp`
> 渲染游戏公开 profile 中的自定义个人信息页，每次渲染一张  
支持服务器: `所有`

- `/自定义个人信息 1` 渲染第 1 页自定义档案
- `/自定义个人信息 u2 3` 渲染绑定列表中第 2 个账号的第 3 页

> 注：当前文字渲染使用思源黑体（游戏专用字体不可得），与游戏内样式可能略有差异；首次渲染需下载档案素材，耗时约 30 秒
...
### 查询角色等级任务
`/cr任务` `/角色等级任务`
> 查询指定角色的角色等级任务进度，需要📡抓包数据
默认查询该角色的角色等级任务总览，添加`all`参数并指定任务名可查询该任务的全量档位、累计需求和累计EXP
其中`队长次数`和`休息室次数`会同时显示普通任务和EX任务
支持服务器: `所有`

- `/cr任务 miku` 查询miku的角色等级任务总览
- `/cr任务 miku all 队长次数` 查询miku队长次数任务的普通+EX全量档位表
- `/cr任务 miku all 休息室` 查询miku休息室任务的普通+EX全量档位表
- `/cr任务 miku all 衣装` 查询miku服装任务全量档位表
...
> 结合目标PT或目标排名、当前PT、歌曲、火数和活动组卡结果生成规划图，需要📡抓包数据  
支持服务器: `所有`

参数说明：
- 目标：必须提供目标 PT 或目标排名。PT 支持 `pt1000w` `目标1200万`，排名支持 `t100` `100名`
...
- 活动：可写 `event154` 指定活动；不写时使用当前活动
- 卡组：可复用活动组卡参数，例如 `顶配` `画布` `已读` `#123 456 789 101 112` `队友综合25w` `队友实效200` `当前`
...
可用附加参数:  
- 排序: `专精排序` `技能等级排序` `获取时间排序`（也支持同义写法如`mr排序` `slv排序` `入手排序`）  
- 获取时间视图: `获取时间一览` / `入手时间一览` / `时间一览`（该模式仅支持单角色查询）  
- 年份分组: 在获取时间视图基础上加 `按年份` / `按年` / `year`  
- 技能类型视图: `技能` / `技能类型` / `技能分类`（该模式仅支持单角色查询）
- 技能值视图: `技能值` / `按技能值` / `按加分值`（该模式仅支持单角色查询）
支持服务器: `所有`  
...
> 查询指定账号背包中的道具及其数量，需要📡抓包数据  
支持服务器: `所有`

空参数默认不展示水晶、火罐、MySekai 材料和记忆，可使用筛选参数单独查询：
- `/查背包` 查询默认视图
- `/查背包 水晶` 只展示水晶
- `/查背包 火罐` 只展示火罐（演出能量道具）
- `/查背包 ms材料` 只展示 MySekai 材料（国服不支持）
- `/查背包 记忆` 只展示记忆（国服不支持）
...
- `/pjsk开启 live music` 开启虚拟Live通知和新曲上线通知 
