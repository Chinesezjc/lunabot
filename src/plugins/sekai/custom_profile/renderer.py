...
from ..honor_render.drawer import compose_full_honor_image_from_loaded_assets, honor_group_uses_scroll_level
from ..honor_render.model import HonorRequest
from .cache import (
    GLYPH_CONTOUR_CACHE,
    GLYPH_SDF_CACHE,
    MISSING,
...
from .honor_deck_prefab import build_honor_deck_plan
from .limits import RasterSizeLimitError, ensure_raster_size
from .svg import (
    CANVAS_H,
    CANVAS_W,
    DEFAULT_ASSETS,
...
# LunaBot 适配：静态图与字体路径
from ..common import SEKAI_ASSET_DIR
SEKAI_STATIC_IMAGES_DIR = f"{SEKAI_ASSET_DIR}/static_images"
SEKAI_BOLD_FONT_PATH = "data/utils/fonts/SourceHanSansCN-Bold.otf"
...
class PNGRenderer:
    ...
    def resolve_static_images_root(self) -> Path:
        ...
        # LunaBot 适配：static_images 位于 rip 资源根下的 static_images/
        candidates.append(Path(SEKAI_STATIC_IMAGES_DIR))
        seen: set[Path] = set()
    ...
    def general_font_candidates(self) -> list[Path]:
        ...
        LunaBot 适配：游戏专有字体不可得，改用思源黑体（Bold）优先。
        """

        if self.region in {"jp", "ja"}:
            return [
                Path(SEKAI_BOLD_FONT_PATH),
    ...
    def honor_request_image(self, payload: dict[str, Any] | None) -> Image.Image | None:
        if not payload:
            return None
        request = HonorRequest(**{k: v for k, v in payload.items() if k in HonorRequest.__dataclass_fields__})
        images = {
    ...
    def omikuji_font_candidates(self, *, decorative: bool = False) -> list[Path]:
        # LunaBot 适配：游戏专用字体不可得，改用思源黑体（Heavy/Bold）
        candidates: list[Path] = [
            Path("data/utils/fonts/SourceHanSansCN-Heavy.otf" if decorative else "data/utils/fonts/SourceHanSansCN-Bold.otf"),
        ]
        names = (
